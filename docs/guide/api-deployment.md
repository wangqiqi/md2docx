...
程序可读出口：`BaseConverter.last_metrics`（字段 `duration_ms` / `input_bytes` / `chunked`）。详见 [开发指南 · 转换指标](/guide/development#转换指标convertmetrics)。CLI `--debug` 会在转换后打印该快照。CI 基线门禁见 [开发指南 · ConvertMetrics 性能基线](/guide/development#convertmetrics-性能基线ci)。
