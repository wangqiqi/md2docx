...
- [ ] **发布候选预检（推荐）**：`bash scripts/verify_release_candidate.sh`  
  聚合 pytest（覆盖率 ≥85%）、flake8、black、mypy、isort、ConvertMetrics、`python -m build` + `twine check`、wheel 卫生检查、隔离安装后 `pip check` / CLI 转换 / WebUI 资源冒烟，并提示 PyPI 版本碰撞（`0.5.48` 已发布时后续 `/release` 须升 `0.5.49`）。
- [ ] 构建包：`python -m build`
- [ ] 检查包：`twine check dist/*`
- [ ] 测试安装：`pip install dist/mddocx-0.5.48-py3-none-any.whl`
...
```bash
bash scripts/verify_pypi_version.sh 0.5.47
# 可选重试：PYPI_RETRIES=3 PYPI_RETRY_SLEEP=5 bash scripts/verify_pypi_version.sh 0.5.48
```
