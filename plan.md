...
**当前状态**：活跃 **M-DOC-04** · **199 passed** · 最新 **v0.5.27** · 🔧 **M-DOC-04-02**

---

## 活跃 Sprint · M-DOC-04（文档与审查清单同步）

> **WHY**：台账 F 组 + 审查行动计划与代码脱节；低风险文档 Sprint，闭合后 ROADMAP #11 ✅

**执行顺序**：M-DOC-04-01 → M-DOC-04-02 → M-DOC-04-03 → M-DOC-04-04 → M-DOC-04-05 → M-DOC-04-06

| ID | 任务 | 优先级 | 状态 | 验收 | 落点 | 备注 |
|----|------|--------|------|------|------|------|
| M-DOC-04-01 | 审查.md 行动计划 checkbox 同步（DOC-01） | P2 | ✅ | `grep -q '\[x\].*BaseConverter' 审查.md` | `审查.md` | P0/P1 全 ✅ |
| M-DOC-04-02 | webui README HOST/安装/CSRF（DOC-04） | P2 | 🔧 | `grep -q '127.0.0.1' src/mddocx/webui/README.md` | `src/mddocx/webui/README.md` | REV-P1-06 |
| M-DOC-04-03 | 删除过时 requirements.txt 引用（REV-P1-07） | P2 | ⬜ | `! grep -q 'requirements\.txt' docs/development.md` | `docs/development.md` | |
| M-DOC-04-04 | README 版本/测试数/路线图同步（DOC-03） | P2 | ⬜ | `grep -q '0.5.26' README.md && grep -q '199' README.md` | `README.md` | REV-P1-05 |
| M-DOC-04-05 | architecture.md 结构刷新（DOC-02） | P2 | ⬜ | `grep -q 'token_processor' docs/architecture.md` | `docs/architecture.md` | REV-P2-05 |
| M-DOC-04-06 | docs/plan.md ROADMAP 同步（DOC-05） | P2 | ⬜ | `grep -q 'M-MATH-01' docs/plan.md` | `docs/plan.md` | |

**闭合条件**：上表 6 项 ✅ · 全量 `pytest -q` · ROADMAP #11 标已完成
...
| # | 主题 | 优先级 | 建议 Sprint | 任务 ID 前缀 |
|---|------|--------|-------------|--------------|
| 9 | M-ARCH 残余（mypy 全量 / 流式转换） | P2 | M-MYPY-01 / M-PERF-01 | BACKLOG-09 |
| 10 | 审查 Minor 代码质量 | P3 | M-MINOR-01 | BACKLOG-10 |
| 11 | 文档与审查清单同步 | P2 | M-DOC-04 | BACKLOG-11 |
| 12 | Mermaid 扩展（sequence / gantt） | P2 | M-MERMAID-02 | BACKLOG-12 |
| 13 | 公式编号与 `\ref` | P2 | M-MATH-02 | BACKLOG-13 |
| 14 | 测试覆盖率与缺口补强 | P2 | T-TEST-02 | BACKLOG-14 |

**下一 Sprint 建议**：**M-DOC-04**（文档同步 + `审查.md` 勾选更新，低风险）或 **M-MYPY-01**（类型债）
...
| REV-P1-05 | CHANGELOG / README 同步 | M7/M10/M11 | 🔶 | 持续更新；README 仍有过时段落 |
| REV-P1-06 | 开发默认 `127.0.0.1` / 生产 SECRET_KEY | M4 | 🔶 | `config.py` 已改；`webui/README.md` 仍写 0.0.0.0 |
| REV-P1-07 | 删除过时 `requirements*.txt` 引用 | M8 | ⬜ | README/docs 扫尾 |
...
| REV-P2-05 | 补齐 `architecture.md` / `testing.md` | m8/m9 / 阶段三 | ⬜ | 部分已更新，未全量审计 |
...
| DOC-04 | 修正 [`src/mddocx/webui/README.md`](src/mddocx/webui/README.md) HOST 说明 | ⬜ | 与 `config.py` 一致 |
...
- **2026-06-09** · **v0.5.27 打版** · jwplan 台账 + jwrun §7B + 文档范围收窄 + 删除 roundtrip demo
