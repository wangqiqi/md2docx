...
class ListConverter(ElementConverter):
    ...
    def convert(self, tokens: Tuple[Any, Any]) -> Paragraph:
        ...
        # 调试信息
        debug = hasattr(self, 'base_converter') and self.base_converter and hasattr(self.base_converter, 'debug') and self.base_converter.debug
        if debug:
            print(f"ListConverter: 处理列表项, token.type={list_token.type}, content={content_token.content if hasattr(content_token, 'content') else 'N/A'}")
