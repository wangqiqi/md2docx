            ...
            while i < len(tokens):
                ...
                # 处理列表项
                    ...
                    if converter:
                        # 获取列表类型和级别
                        list_type = (
                            self._base._list_stack[-1][0] if self._base._list_stack else "bullet"
                        )
                        level = self._base._list_stack[-1][1] if self._base._list_stack else 1

                        if self._base.debug:
                            print(
                                f"处理列表项: 栈={self._base._list_stack}, list_type={list_type}, level={level}"
                            )
                        ...
                        if is_task_list and "task_list" in self._base.converters:
                            self._base.converters["task_list"].convert(
